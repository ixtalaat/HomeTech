...
This file gives any AI coding agent (Claude Code, Laravel Boost-powered agents, etc.) or human contributor the context needed to work correctly on the **HomeTech** codebase: a home maintenance service management platform. Read this before generating or modifying code. It merges the project's domain rules with the Laravel Boost operating guidelines already configured for this app.

## 1. Project Summary
 
HomeTech manages the full lifecycle of a home maintenance job: customer request → admin review → technician assignment → scheduling → work order (diagnosis/labor/materials) → additional work approval → invoicing → payment → closure → review.
 
Four roles: **Customer**, **Admin**, **Technician**, **Manager**. Authorization must always be enforced server-side, never trusted from the frontend.
 
Full domain detail lives in `docs/PRD.md`. This file is the *operating contract* for how to build it.
...
| ORM | Eloquent |
| Database | MySQL |
| Auth / Authorization | Laravel auth + Policies/Gates + role-permission system |
| Validation | Laravel Form Requests + explicit server-side business-rule checks |
| Background work | Laravel Queues + Scheduler |
| Notifications | Laravel Notifications (database channel for MVP) |
| Testing | Pest — feature tests + unit tests for business rules |
| Code style | Laravel Pint |
| Frontend (MVP) | Blade + HTML/CSS/JS (+ Bootstrap or similar) |
| Tooling | Laravel Boost MCP server |
...
- After each completed feature or fix, stage the relevant changes and create a focused commit.
