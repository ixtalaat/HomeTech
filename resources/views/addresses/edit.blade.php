...
@section('title', __('Edit Address: :title', ['title' => $address->title]))
...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">{{ __('Edit Address: :title', ['title' => $address->title]) }}</h2>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <form method="POST" action="{{ route('addresses.update', $address) }}" class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                ...
                <button type="submit" class="primary-button">{{ __('Update Address') }}</button>
            </div>
        </form>
    </div>
</div>
