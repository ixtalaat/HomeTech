...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">Add New Category</h2>
        <p class="mt-1 text-sm text-slate-500">Create a service category for grouping home maintenance jobs.</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
            ...
            <div>
                <label for="name" class="form-label">Category Name <span class="text-rose-500">*</span></label>
            ...
            <div>
                ...
                <input type="text" id="slug" name="slug" value="{{ old('slug') }}"
                    ...
                    placeholder="e.g. air-conditioning (leave blank to auto-generate)">
                @error('slug')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    ...
                    placeholder="Brief description of the services offered in this category...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="icon" class="form-label">Icon identifier (Optional)</label>
                <input type="text" id="icon" name="icon" value="{{ old('icon') }}"
                    class="form-input @error('icon') border-rose-300 ring-rose-100 @enderror"
                    placeholder="e.g. snowflake, wrench, zap">
                @error('icon')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <label for="is_active" class="text-sm font-semibold text-slate-700">Category is active and visible</label>
            </div>

            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                <a href="{{ route('admin.categories.index') }}" class="secondary-button">Cancel</a>
