...
<div class="mx-auto max-w-7xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-extrabold text-slate-900">{{ __('Service Categories') }}</h2>
            <p class="mt-1 text-sm text-slate-500">{{ __('Manage categories that group maintenance services and technician skills.') }}</p>
        </div>
                        ...
                        <tr class="hover:bg-slate-50/70 transition">
                            ...
                            <td class="px-6 py-4 text-end">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" data-confirm="{{ __('Are you sure you want to delete this category?') }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                                {{ __('No categories created yet. Click "Add New Category" to create one.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
