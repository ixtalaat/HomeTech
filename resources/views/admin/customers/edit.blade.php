...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">Edit Customer: {{ $customer->name }}</h2>
        <p class="mt-1 text-sm text-slate-500">Update profile details or activation status.</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.customers.update', $customer) }}" class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                <a href="{{ route('admin.customers.show', $customer) }}" class="secondary-button">Cancel</a>
                <button type="submit" class="primary-button">Update Customer</button>
            </div>
        </form>
    </div>
</div>
