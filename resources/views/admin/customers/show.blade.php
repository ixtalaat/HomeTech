...
<div class="mx-auto max-w-4xl">
    <div class="mb-8">
        ...
        <p class="mt-1 text-sm text-slate-500">{{ $customer->email }} · {{ __('Joined :date', ['date' => $customer->created_at->format('d M Y')]) }}</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm">
            <div>
                <dt class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('Name') }}</dt>
                <dd class="mt-1 font-semibold text-slate-900">{{ $customer->name }}</dd>
            </div>
            <div>
                <dt class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('Phone') }}</dt>
                <dd class="mt-1 font-semibold text-slate-900">{{ $customer->phone ?? '—' }}</dd>
            </div>
            <div>
                <dt class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('Email') }}</dt>
                <dd class="mt-1 font-semibold text-slate-900">{{ $customer->email }}</dd>
            </div>
            <div>
                <dt class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('Addresses') }}</dt>
                <dd class="mt-1 font-semibold text-slate-900">{{ $customer->addresses->count() }}</dd>
            </div>
        </dl>
        <div class="mt-6 flex items-center gap-2">
            <a href="{{ route('admin.customers.edit', $customer) }}" class="secondary-button text-xs">{{ __('Edit Customer') }}</a>
            <form method="POST" action="{{ route('admin.customers.toggle-status', $customer) }}" class="inline">
                @csrf
                @method('PATCH')
                <button type="submit" class="secondary-button text-xs">{{ $customer->is_active ? __('Deactivate') : __('Activate') }}</button>
            </form>
        </div>
    </div>

    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h3 class="text-sm font-extrabold text-slate-900">{{ __('Addresses (:count)', ['count' => $customer->addresses->count()]) }}</h3>
        @forelse ($customer->addresses as $address)
            <div class="mt-4 rounded-xl border border-slate-100 p-4">
                <div class="flex items-center justify-between">
                    <p class="font-bold text-slate-900">{{ $address->title }}</p>
                    @if($address->is_default)
                        <span class="rounded-full bg-teal-50 px-2.5 py-1 text-xs font-bold text-teal-700">{{ __('Default') }}</span>
                    @endif
                </div>
        ...
        @empty
            <p class="mt-3 text-sm text-slate-500">{{ __('No addresses saved yet.') }}</p>
        @endforelse
    </div>
</div>
@endsection
