...
<div class="mx-auto max-w-7xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-extrabold text-slate-900">Inventory Management</h2>
            <p class="mt-1 text-sm text-slate-500">Track materials, stock levels, and every movement.</p>
        </div>
    ...
    @if($lowStockItems->isNotEmpty())
        <div class="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
            <h3 class="text-sm font-extrabold text-amber-900">⚠ Low Stock ({{ $lowStockItems->count() }} items at or below threshold)</h3>
                ...
                @foreach ($lowStockItems as $lowItem)
                    <li class="flex items-center justify-between rounded-xl bg-white/70 px-3 py-2">
                        ...
                        <span class="text-xs font-bold {{ $lowItem->current_stock === 0 ? 'text-rose-600' : 'text-amber-700' }}">
                            {{ $lowItem->current_stock }} {{ $lowItem->unit }} left
                        </span>
                    </li>
                @endforeach
            </ul>
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.inventory.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div class="sm:w-64">
                <label for="search" class="form-label text-xs">Search Item</label>
                <input type="text" id="search" name="search" value="{{ request('search') }}"
                    placeholder="Name or SKU..." class="form-input text-xs py-2">
            </div>
            <div class="sm:w-48">
                <label for="filter" class="form-label text-xs">Filter</label>
                <select id="filter" name="filter" class="form-input text-xs py-2" onchange="this.form.submit()">
                    <option value="">All Items</option>
                    <option value="low-stock" {{ request('filter') === 'low-stock' ? 'selected' : '' }}>Low Stock Only</option>
                </select>
            </div>
            <div class="flex items-center gap-2">
                <button type="submit" class="primary-button text-xs py-2 px-4">Filter</button>
                @if(request()->hasAny(['search', 'filter']))
                    <a href="{{ route('admin.inventory.index') }}" class="secondary-button text-xs py-2 px-3">Reset</a>
                @endif
            </div>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($items as $item)
                        <tr class="hover:bg-slate-50/70 transition">
                            ...
                            <td class="px-6 py-4 text-xs">{{ number_format($item->unit_cost, 2) }} EGP</td>
                            <td class="px-6 py-4">
                                @if($item->isLowOnStock())
                                    <span class="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-800">Low Stock</span>
                                @else
                                    <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">OK</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500">No inventory items found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
