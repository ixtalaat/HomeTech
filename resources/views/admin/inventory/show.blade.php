...
<div class="mx-auto max-w-4xl">
    <div class="mb-8">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-3">
            ...
            @if($inventoryItem->isLowOnStock())
                <span class="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-800">Low Stock</span>
            @endif
        </div>
        <p class="mt-1 text-sm text-slate-500">
            {{ $inventoryItem->current_stock }} {{ $inventoryItem->unit }} in stock ·
            {{ number_format($inventoryItem->unit_cost, 2) }} EGP/{{ $inventoryItem->unit }} ·
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 class="text-sm font-extrabold text-slate-900">Manual Adjustment</h3>
            <p class="mt-1 text-xs text-slate-500">Positive restocks, negative corrects. Never below zero; every change is logged.</p>
            <form method="POST" action="{{ route('admin.inventory.adjust', $inventoryItem) }}" class="mt-3 space-y-3">
                @csrf
                @method('PATCH')
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label for="delta" class="form-label text-xs">Quantity Change</label>
                ...
                <button type="submit" class="secondary-button w-full text-xs">Apply Adjustment</button>
            </form>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 class="text-sm font-extrabold text-slate-900">Movement Ledger</h3>
            <ol class="mt-3 space-y-3">
                ...
                @empty
                    <p class="text-sm text-slate-500">No movements recorded.</p>
                @endforelse
            </ol>
        </div>
    </div>
