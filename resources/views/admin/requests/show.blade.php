...
<div class="mx-auto max-w-5xl">
    <div class="mb-8">
        ...
        @if($maintenanceRequest->workOrder)
            <a href="{{ route('admin.work-orders.show', $maintenanceRequest->workOrder) }}" class="mt-2 inline-flex items-center gap-1 text-xs font-bold text-teal-700 hover:underline">
                {{ __('View work order #:id (:status) →', ['id' => $maintenanceRequest->workOrder->id, 'status' => $maintenanceRequest->workOrder->status->label()]) }}
            </a>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    ...
                    @if(! empty($maintenanceRequest->photos))
                        <div>
                            <dt class="text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('Photos') }}</dt>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 class="text-sm font-extrabold text-slate-900">{{ __('Appointment') }}</h3>
                ...
                @if($maintenanceRequest->isReviewable())
                    <form method="POST" action="{{ route('admin.requests.appointment', $maintenanceRequest) }}" class="mt-4 space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="preferred_date" class="form-label text-xs">{{ __('Date') }}</label>
                            ...
                            <div>
                                <label for="preferred_time" class="form-label text-xs">{{ __('Time') }}</label>
                        ...
                        <button type="submit" class="secondary-button text-xs">{{ __('Update Appointment') }}</button>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-6">
            @if($maintenanceRequest->isReviewable())
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <form method="POST" action="{{ route('admin.requests.approve', $maintenanceRequest) }}" class="mt-4 space-y-3">
                        ...
                        @method('PATCH')
                        <div>
                            <label for="approve_note" class="form-label text-xs">{{ __('Approval Note (optional)') }}</label>
                    ...
                    <form method="POST" action="{{ route('admin.requests.request-info', $maintenanceRequest) }}" class="mt-4 space-y-3 border-t border-slate-100 pt-4">
                        ...
                        @method('PATCH')
                        <div>
                            <label for="info_note" class="form-label text-xs">{{ __('What do you need from the customer?') }} <span class="text-rose-500">*</span></label>
                    ...
                    <form method="POST" action="{{ route('admin.requests.reject', $maintenanceRequest) }}" class="mt-4 space-y-3 border-t border-slate-100 pt-4" data-confirm="{{ __('Reject this request?') }}">
                        @csrf
                        @method('PATCH')
                        <div>
                            <label for="rejection_reason" class="form-label text-xs">{{ __('Rejection Reason') }} <span class="text-rose-500">*</span></label>
                            <textarea id="rejection_reason" name="rejection_reason" rows="2" required class="form-input text-xs"></textarea>
                        </div>
                        <button type="submit" class="w-full rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 transition">{{ __('Reject Request') }}</button>
                    </form>
                </div>
            @else
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm text-sm text-slate-500">
                    {{ __('This request has been reviewed (:status) and can no longer be changed here.', ['status' => $maintenanceRequest->status->label()]) }}
                    @if($maintenanceRequest->rejection_reason)
                        <p class="mt-2 font-semibold text-slate-700">{{ __('Reason: :reason', ['reason' => $maintenanceRequest->rejection_reason]) }}</p>
                    @endif
                </div>
            @endif
            ...
            @if($maintenanceRequest->status === \App\Enums\RequestStatus::Approved)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-xs text-slate-500">{{ __('Assigning books the preferred slot automatically (end derived from the service duration).') }}</p>
                    <form method="POST" action="{{ route('admin.requests.assign', $maintenanceRequest) }}" class="mt-4 space-y-3">
                        @csrf
                        @method('PATCH')
                        <div>
                            <label for="technician_id" class="form-label text-xs">{{ __('Assign technician') }}</label>
                            <select id="technician_id" name="technician_id" required class="form-input text-xs">
                                ...
                                @foreach ($eligibleTechnicians as $technician)
                                    <option value="{{ $technician->id }}">
                                        {{ $technician->user->name }} ({{ $technician->assigned_requests_count ?? 0 }} {{ __('Assigned') }} · {{ $technician->day_load ?? 0 }}/{{ \App\Services\TechnicianAssignmentService::MAX_DAILY_REQUESTS }} {{ $maintenanceRequest->preferred_date->format('d M') }}){{ ! empty($technician->slot_note) ? ' — '.$technician->slot_note : '' }}
                                    </option>
                                @endforeach
                            </select>
                            @if($eligibleTechnicians->isEmpty())
                                <p class="mt-1.5 text-xs font-semibold text-amber-600">{{ __('No eligible technicians: none are active with skills for this service category.') }}</p>
                            @endif
                        </div>
                        <button type="submit" class="primary-button w-full text-xs">{{ __('Assign & Book') }}</button>
                    </form>
                </div>
            @endif
            ...
            @if($maintenanceRequest->status === \App\Enums\RequestStatus::TechnicianAssigned)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h3 class="text-sm font-extrabold text-slate-900">{{ __('Book Appointment') }}</h3>
                    <p class="mt-2 text-sm text-slate-600">
                        {{ __('Assigned to :name — awaiting a slot.', ['name' => $maintenanceRequest->technician->user->name ?? '—']) }}
                    </p>
            ...
            @if(in_array($maintenanceRequest->status, [\App\Enums\RequestStatus::Scheduled, \App\Enums\RequestStatus::TechnicianOnWay], true) && $maintenanceRequest->appointment)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <form method="POST" action="{{ route('admin.requests.reschedule-appointment', $maintenanceRequest) }}" class="mt-4 space-y-3 border-t border-slate-100 pt-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                <label for="re_date" class="form-label text-xs">{{ __('Date') }}</label>
                            ...
                            <div>
                                <label for="re_start" class="form-label text-xs">{{ __('Start') }}</label>
                            ...
                            <div>
                                <label for="re_end" class="form-label text-xs">{{ __('End') }}</label>
                        ...
                        <button type="submit" class="secondary-button w-full text-xs">{{ __('Reschedule') }}</button>
                    </form>
                    <div class="mt-2 flex gap-2">
                        <form method="POST" action="{{ route('admin.requests.cancel-appointment', $maintenanceRequest) }}" data-confirm="{{ __('Cancel this appointment?') }}" class="flex-1">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="w-full rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 transition">{{ __('Cancel Appointment') }}</button>
                        </form>
                    </div>
                    <form method="POST" action="{{ route('admin.requests.assign', $maintenanceRequest) }}" class="mt-2 space-y-2">
                        @csrf
                        @method('PATCH')
                        <select name="technician_id" required class="form-input text-xs" aria-label="{{ __('Reassign technician') }}">
                            <option value="">{{ __('Reassign to…') }}</option>
                            @foreach ($eligibleTechnicians as $technician)
                                <option value="{{ $technician->id }}">{{ $technician->user->name }} ({{ $technician->day_load ?? 0 }}/{{ \App\Services\TechnicianAssignmentService::MAX_DAILY_REQUESTS }}){{ ! empty($technician->slot_note) ? ' — '.$technician->slot_note : '' }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="secondary-button w-full text-xs">{{ __('Reassign') }}</button>
                    </form>
                </div>
            @endif

            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 class="text-sm font-extrabold text-slate-900">{{ __('Billing') }}</h3>
                ...
                @elseif($maintenanceRequest->workOrder && $maintenanceRequest->status === \App\Enums\RequestStatus::Completed)
                    <form method="POST" action="{{ route('admin.invoices.generate', $maintenanceRequest->workOrder) }}" class="mt-3">
                        @csrf
                        <button type="submit" class="primary-button w-full text-xs">{{ __('Generate Invoice') }}</button>
                    </form>
                @else
                    <p class="mt-2 text-xs text-slate-500">{{ __('No invoice yet — invoicing unlocks after the job is completed.') }}</p>
                @endif
            </div>
            ...
            @if(!$maintenanceRequest->cancellation && in_array($maintenanceRequest->status, [\App\Enums\RequestStatus::PendingReview, \App\Enums\RequestStatus::InfoRequested, \App\Enums\RequestStatus::Approved, \App\Enums\RequestStatus::TechnicianAssigned, \App\Enums\RequestStatus::Scheduled], true))
                <div class="rounded-2xl border border-rose-200 bg-rose-50/50 p-6 shadow-sm">
                    <h3 class="text-sm font-extrabold text-slate-900">{{ __('Cancel Request') }}</h3>
                    <form method="POST" action="{{ route('admin.requests.cancel', $maintenanceRequest) }}" class="mt-3 space-y-3" data-confirm="{{ __('Cancel this request per the policy?') }}">
                        @csrf
                        @method('PATCH')
                        <textarea name="reason" rows="2" required placeholder="{{ __('Cancellation reason…') }}" class="form-input text-xs"></textarea>
                        <button type="submit" class="w-full rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 transition">{{ __('Cancel Request') }}</button>
                    </form>
                </div>
            @endif
