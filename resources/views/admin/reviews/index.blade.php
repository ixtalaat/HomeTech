...
<div class="mx-auto max-w-7xl">
    <div class="mb-8">
        <h2 class="font-display text-2xl font-extrabold text-slate-900">Customer Reviews</h2>
        <p class="mt-1 text-sm text-slate-500">Feedback submitted after completed jobs.</p>
    </div>

    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.reviews.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div class="sm:w-48">
                <label for="rating" class="form-label text-xs">Filter by Rating</label>
                <select id="rating" name="rating" class="form-input text-xs py-2" onchange="this.form.submit()">
                    <option value="">All Ratings</option>
                    @for($stars = 5; $stars >= 1; $stars--)
                        <option value="{{ $stars }}" {{ (int) request('rating') === $stars ? 'selected' : '' }}>{{ $stars }} stars</option>
                    @endfor
                </select>
            </div>
            @if(request('rating'))
                <a href="{{ route('admin.reviews.index') }}" class="secondary-button text-xs py-2 px-3">Reset</a>
            @endif
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-500">No reviews yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
