...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">{{ __('Add New Service') }}</h2>
        <p class="mt-1 text-sm text-slate-500">{{ __('Add a new home maintenance service with base price and estimated duration.') }}</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                    ...
                    placeholder="{{ __('e.g. AC Filter Wash & Deep Cleaning') }}">
                @error('name')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    ...
                    placeholder="{{ __('Describe what is included in this service, typical diagnosis, and prerequisites...') }}">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs font-semibold text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                ...
                <button type="submit" class="primary-button">{{ __('Create Service') }}</button>
            </div>
        </form>
    </div>
</div>
