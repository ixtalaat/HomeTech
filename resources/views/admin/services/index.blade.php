...
<div class="mx-auto max-w-7xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-extrabold text-slate-900">Services Management</h2>
            <p class="mt-1 text-sm text-slate-500">Configure maintenance services, base pricing, durations, and active statuses.</p>
        </div>
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
        <form method="GET" action="{{ route('admin.services.index') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-4 items-end">
            <div>
                <label for="search" class="form-label text-xs">Search Service</label>
                <input type="text" id="search" name="search" value="{{ request('search') }}"
                    placeholder="Search by name..." class="form-input text-xs py-2">
            </div>

            <div>
                <label for="category_id" class="form-label text-xs">Category</label>
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'category_id', 'status']))
                    <a href="{{ route('admin.services.index') }}" class="secondary-button text-xs py-2 px-3">Reset</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-600">
                <thead class="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4">Base Price</th>
                        <th scope="col" class="px-6 py-4">Est. Duration</th>
                        ...
                        <tr class="hover:bg-slate-50/70 transition">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.services.destroy', $service) }}" onsubmit="return confirm('Are you sure you want to delete this service?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
