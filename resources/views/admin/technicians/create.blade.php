...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">{{ __('Add Technician') }}</h2>
        <p class="mt-1 text-sm text-slate-500">{{ __('Creates a technician user account together with the employment profile.') }}</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.technicians.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    <label for="password" class="form-label">{{ __('Initial Password') }} <span class="text-rose-500">*</span></label>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" id="hired_at" name="hired_at" value="{{ old('hired_at') }}"
                    ...
                    @foreach ($categories as $category)
                        <label class="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700">
                            <input type="checkbox" name="skills[]" value="{{ $category->id }}"
                                {{ in_array($category->id, old('skills', [])) ? 'checked' : '' }}
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                <a href="{{ route('admin.technicians.index') }}" class="secondary-button">{{ __('Cancel') }}</a>
                <button type="submit" class="primary-button">{{ __('Create Technician') }}</button>
            </div>
        </form>
    </div>
</div>
