...
<div class="mx-auto max-w-7xl">
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
        <form method="GET" action="{{ route('branch.requests.index') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end">
            <div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('branch.requests.index') }}" class="secondary-button text-xs py-2 px-3">{{ __('Reset') }}</a>
                @endif
            </div>
        </form>
    </div>
                    ...
                    @forelse ($requests as $maintenanceRequest)
                        <tr class="hover:bg-slate-50/70 transition">
                            ...
                            <td class="px-6 py-4 text-end">
                                <a href="{{ route('branch.requests.show', $maintenanceRequest) }}"
