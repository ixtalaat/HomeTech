...
@section('content')
    <div class="mx-auto max-w-6xl">
        <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p class="text-sm font-semibold text-slate-500">{{ __('Here’s what’s happening with your home.') }}</p><h2 class="mt-2 font-display text-3xl font-extrabold tracking-tight text-slate-950">{{ __('Stay ahead of every repair.') }}</h2></div><a href="{{ route('requests.create') }}" class="primary-button">{{ __('Request a service') }} <span aria-hidden="true">＋</span></a></div>
        <div class="mt-8 grid gap-4 sm:grid-cols-3">
            <a href="{{ route('requests.index') }}" class="stat-card transition hover:border-teal-200 hover:shadow-md" aria-label="{{ __('View active services') }}">
                <span class="stat-icon bg-teal-100 text-teal-700">✓</span>
                <div><p class="text-sm font-semibold text-slate-500">{{ __('Active services') }}</p><p class="mt-1 font-display text-3xl font-extrabold">{{ $active_services }}</p></div>
            </a>
            <a href="{{ route('requests.index') }}" class="stat-card transition hover:border-amber-200 hover:shadow-md" aria-label="{{ __('View upcoming visits') }}">
                <span class="stat-icon bg-amber-100 text-amber-700">◷</span>
                <div><p class="text-sm font-semibold text-slate-500">{{ __('Upcoming visits') }}</p><p class="mt-1 font-display text-3xl font-extrabold">{{ $upcoming_visits }}</p></div>
            </a>
            <a href="{{ route('requests.index') }}" class="stat-card transition hover:border-sky-200 hover:shadow-md" aria-label="{{ __('View completed jobs') }}">
                <span class="stat-icon bg-sky-100 text-sky-700">▣</span>
                <div><p class="text-sm font-semibold text-slate-500">{{ __('Completed jobs') }}</p><p class="mt-1 font-display text-3xl font-extrabold">{{ $completed_jobs }}</p></div>
            </a>
        </div>
                                ...
                                <a href="{{ route('requests.show', $recent) }}" class="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3 transition hover:bg-teal-50">
                                    <span>
                                        ...
                                        <span class="mt-0.5 block text-xs text-slate-500">
                                            ...
                                            @else
                                                {{ __('Preferred :date', ['date' => $recent->preferred_date->format('d M Y')]) }}
                                            @endif
                                        </span>
                                    </span>
