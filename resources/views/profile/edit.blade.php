...
@section('content')
    ...
    <div class="mb-6 rounded-3xl border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <p class="text-sm font-bold uppercase tracking-[0.14em] text-teal-600">Phone verification</p>
                <p class="mt-1 text-sm text-slate-500">
                    @if($user->phone_verified_at)
                        <span class="font-bold text-emerald-600">✓ {{ $user->phone }} is verified.</span>
                    @elseif($user->phone)
                        {{ $user->phone }} is not verified yet. Confirm it via WhatsApp.
                    @else
                        Add a phone number above first, then verify it via WhatsApp.
                    @endif
                </p>
            </div>
            @unless($user->phone_verified_at)
                <form method="POST" action="{{ route('phone.send-code') }}" class="inline">
                    @csrf
                    <button type="submit" class="secondary-button text-xs">Send WhatsApp Code</button>
                </form>
            @endunless
        </div>
        @unless($user->phone_verified_at)
            <form method="POST" action="{{ route('phone.verify') }}" class="mt-4 flex flex-col gap-3 sm:flex-row">
                @csrf
                <input type="text" name="code" inputmode="numeric" maxlength="6" required placeholder="6-digit code" class="form-input sm:w-48">
                <button type="submit" class="secondary-button text-xs">Verify Code</button>
            </form>
        @endunless
    </div>
