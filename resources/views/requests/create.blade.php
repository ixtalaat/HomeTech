...
<div class="mx-auto max-w-3xl">
    <div class="mb-8">
        ...
        <h2 class="mt-2 font-display text-2xl font-extrabold text-slate-900">New Maintenance Request</h2>
        <p class="mt-1 text-sm text-slate-500">Describe the problem, pick a service address, and choose a preferred appointment.</p>
    </div>

    @if($addresses->isEmpty())
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm font-medium text-amber-900">
            You need a service address first.
            <a href="{{ route('addresses.create') }}" class="font-bold underline">Add an address</a> to continue.
        </div>
    @else
        <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            <form method="POST" action="{{ route('requests.store') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <select id="service_id" name="service_id" required
                        ...
                        @foreach ($services as $service)
                            <option value="{{ $service->id }}" {{ (string) old('service_id', $selectedService) === (string) $service->id ? 'selected' : '' }}>
                                {{ $service->name }} — {{ number_format($service->base_price, 2) }} EGP
                            </option>
                        @endforeach
                    </select>
                    @error('service_id')
                ...
                <div>
                    <label for="address_id" class="form-label">Service Address <span class="text-rose-500">*</span></label>
                    <select id="address_id" name="address_id" required
                        ...
                        @foreach ($addresses as $address)
                            <option value="{{ $address->id }}" {{ (string) old('address_id') === (string) $address->id ? 'selected' : '' }}>
                                {{ $address->title }} — {{ $address->street }}, {{ $address->city }}{{ $address->is_default ? ' (Default)' : '' }}
                            </option>
                        @endforeach
                    </select>
                    @error('address_id')
                ...
                <div>
                    <label for="description" class="form-label">Problem Description <span class="text-rose-500">*</span></label>
                    <textarea id="description" name="description" rows="4" required placeholder="e.g. My AC is running but is not cooling the room."
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        <label for="preferred_date" class="form-label">Preferred Date <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label for="preferred_time" class="form-label">Preferred Time <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label for="photos" class="form-label">Photos (optional, up to 5)</label>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                    <a href="{{ route('requests.index') }}" class="secondary-button">Cancel</a>
