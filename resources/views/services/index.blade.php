...
<head>
    ...
    <title>Available Services — HomeTech</title>
            ...
            <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
                <div class="max-w-2xl">
                    ...
                    <h1 class="mt-4 font-display text-4xl font-extrabold tracking-tight text-slate-950 sm:text-5xl">
                        Expert care for every part of your home.
                    </h1>
                    <p class="mt-4 text-base leading-7 text-slate-600">
                        Transparent pricing, certified technicians, and hassle-free scheduling across all home maintenance categories.
                    </p>
                </div>
                    ...
                    <form method="GET" action="{{ route('services.index') }}" class="flex gap-2">
                        ...
                        <div class="relative flex-1">
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Search services (e.g. AC cleaning, faucet repair)..."
                        ...
                        <button type="submit" class="primary-button text-xs px-5">Search</button>
                    </form>
                </div>
            </div>
        </section>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @forelse ($services as $service)
                        <div class="flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-teal-300 hover:shadow-md">
                            ...
                            <p class="mt-2 flex-1 text-sm leading-6 text-slate-500 line-clamp-3">
                                {{ $service->description ?? 'Professional maintenance and diagnostics by qualified technicians.' }}
                            </p>

                            <div class="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
                                <div>
                                    <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Starting from</p>
                                    <p class="font-display text-xl font-extrabold text-slate-950">
                                        {{ number_format($service->base_price, 2) }} <span class="text-xs font-semibold text-slate-500">EGP</span>
                                    </p>
                                </div>
                    ...
                    @empty
                        <div class="col-span-full rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center">
                            ...
                            <h3 class="mt-3 text-base font-bold text-slate-900">No services found</h3>
                            <p class="mt-1 text-sm text-slate-500">Try adjusting your category filter or search keywords.</p>
                            <div class="mt-5">
                                <a href="{{ route('services.index') }}" class="primary-button text-xs py-2 px-4">Clear all filters</a>
                            </div>
                        </div>
                    @endforelse
                </div>
