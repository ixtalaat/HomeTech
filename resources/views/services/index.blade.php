        ...
        <div class="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <nav class="flex items-center gap-2 text-sm font-bold sm:gap-4">
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="primary-button hidden py-2 px-4 text-xs whitespace-nowrap sm:inline-flex">{{ __('Get started') }} <span aria-hidden="true">→</span></a>
                @endauth
                <x-mobile-nav active="services" />
            </nav>
        </div>
    </header>
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-1 text-sm no-scrollbar">
                    ...
                    @foreach ($categories as $cat)
                        <a href="{{ route('services.index', array_merge(request()->only('search'), ['category' => $cat->slug])) }}"
                            ...
                            {{ $cat->display_name }}
                            <span class="ms-1 opacity-70">({{ $cat->services_count }})</span>
                        </a>
                    @endforeach
                </div>
            </div>
