...
<body class="bg-slate-50 text-slate-900 antialiased min-h-screen flex flex-col">
    ...
    <header class="relative z-10 border-b border-slate-200/80 bg-white/90 px-5 py-4 backdrop-blur sm:px-8 lg:px-12">
        <div class="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <nav class="flex items-center gap-4 text-sm font-bold">
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-xl border border-slate-200 px-4 py-2 text-slate-700 hover:border-teal-200 hover:bg-teal-50 transition">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded-xl px-4 py-2 text-slate-600 transition hover:text-slate-950">Sign in</a>
                    <a href="{{ route('register') }}" class="primary-button py-2 px-4 text-xs">Get started <span aria-hidden="true">→</span></a>
                @endauth
            </nav>
        </div>
    </header>
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-[1.2fr_0.8fr]">
                ...
                <div>
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h2 class="text-base font-bold text-slate-900">Service Description & Scope</h2>
                        <div class="mt-3 text-sm leading-7 text-slate-600 space-y-3">
                            <p>{{ $service->description ?? 'Professional maintenance, thorough diagnostics, and reliable execution by experienced technicians.' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="stat-card">
                            ...
                            <div>
                                <p class="text-xs font-semibold text-slate-400">Estimated Duration</p>
                                <p class="font-display text-lg font-bold text-slate-900">~{{ $service->estimated_duration_minutes }} minutes</p>
                            </div>
                        </div>
                        ...
                        <div class="stat-card">
                            ...
                            <div>
                                <p class="text-xs font-semibold text-slate-400">Quality Guarantee</p>
                                <p class="font-display text-lg font-bold text-slate-900">Verified Pros & Warranty</p>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($relatedServices->isNotEmpty())
                        <div class="mt-12">
                            <h3 class="font-display text-xl font-bold text-slate-900">Other {{ $service->category->name }} Services</h3>
                                ...
                                @foreach ($relatedServices as $related)
                                    <a href="{{ route('services.show', $related->slug) }}"
                                        ...
                                        <div class="mt-3 flex items-center justify-between text-xs">
                                            <span class="font-bold text-slate-900">{{ number_format($related->base_price, 2) }} EGP</span>
                                            <span class="text-teal-700 font-semibold">View →</span>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                ...
                <div>
                    <div class="sticky top-6 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xl shadow-slate-900/5">
                        <p class="text-xs font-bold uppercase tracking-wider text-teal-700">Pricing & Booking</p>
                        
                        <div class="mt-4 flex items-baseline gap-2">
                            <span class="font-display text-4xl font-extrabold text-slate-950">{{ number_format($service->base_price, 2) }}</span>
                            <span class="text-sm font-bold text-slate-500">EGP</span>
                            <span class="text-xs text-slate-400">(base diagnostic & labor estimate)</span>
                        </div>
                        ...
                        <div class="mt-6 border-t border-slate-100 pt-6 space-y-3 text-xs text-slate-600">
                            <div class="flex items-center gap-2">
                                ...
                                <span>No hidden fees — transparent upfront pricing</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="h-4 w-4 text-teal-600 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" /></svg>
                                <span>Scheduled appointment at your preferred date/time</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="h-4 w-4 text-teal-600 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" /></svg>
                                <span>Itemized diagnosis, labor, and materials breakdown</span>
                            </div>
                        </div>
                        ...
                        <div class="mt-8">
                            ...
                            @else
                                <a href="{{ route('register') }}" class="primary-button w-full py-3.5 text-center text-sm font-bold">
                                    Sign Up to Request Service
                                </a>
                                <p class="mt-2 text-center text-xs text-slate-500">
                                    Already have an account? <a href="{{ route('login') }}" class="font-bold text-teal-700 hover:underline">Log in</a>
                                </p>
                            @endauth
                        </div>
                    </div>
    ...
    <footer class="border-t border-slate-200 bg-white">
        <div class="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-8 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12">
            ...
            <span>Built for better everyday living.</span>
        </div>
    </footer>
</body>
</html>
